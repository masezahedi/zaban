...
const WordsPage: React.FC = () => {
  ...
  const handleUpdateCategory = async () => {
    ...
    try {
      await updateCategory(editingCategory.id!, user.id, newCategoryTitle.trim());
      await loadData();
  ...
  const handleDeleteCategory = async (categoryId: number) => {
    ...
    if (!confirm('آیا از حذف این دسته‌بندی اطمینان دارید؟ تمام لغات این دسته‌بندی نیز حذف خواهند شد.')) {
      return;
    }
  ...
  const handleAddWord = async () => {
    if (!user?.id || !newWord.word.trim() || !newWord.translation.trim() || !newWord.categoryId) return;
  ...
  const handleUploadWords = async () => {
    ...
    try {
      ...
      try {
        ...
        throw new Error('فایل JSON معتبر نیست. لطفاً مطمئن شوید که فایل شما یک JSON معتبر است و هیچ کاراکتر نامعتبری ندارد.');
      }
      ...
      if (invalidWords.length > 0) {
        throw new Error(`${invalidWords.length} لغت با فرمت نامعتبر یافت شد. هر لغت باید دارای فیلدهای 'word' و 'translate' باشد.`);
      }
      ...
      toast.error(error instanceof Error ? error.message : 'خطا در خواندن فایل یا افزودن لغات');
    } finally {
  ...
  const handleAddSelectedWords = async () => {
    ...
    try {
      const selectedWords = randomWords.filter(w => selectedRandomWords.has(w.id!));
            ...
            <div className="card">
              ...
              {showWordBank && (
                <div className="mb-6 p-4 bg-gray-50 rounded-lg">
                  <h3 className="text-lg font-bold text-gray-900 mb-4">افزودن از بانک لغات</h3>
                  ...
                  <div className="flex justify-end mb-4">
                    <button
                      ...
                      className="btn-primary"
                    >
                      {isLoadingRandom ? 'در حال دریافت...' : 'دریافت لغات تصادفی'}
                    </button>
                  </div>
                        ...
                        {randomWords.map((word) => (
                          <div
                            ...
                            className={`
                              ...
                              ${selectedRandomWords.has(word.id!) ? 'bg-primary-100' : 'bg-white'}
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-gray-200">
                      ...
                      <th className="text-right py-3 px-4 hidden md:table-cell">کاربرد</th>
                      <th className="text-right py-3 px-4 hidden md:table-cell">دسته‌بندی</th>
                  ...
                  <tbody>
                    {words.map((word) => (
                      <tr key={word.id} className="border-b border-gray-100 hover:bg-gray-50">
                        ...
                        <td className="py-3 px-4 hidden md:table-cell">{word.usage}</td>
                        <td className="py-3 px-4 hidden md:table-cell">
                          {categories.find(c => c.id === word.categoryId)?.title}
                        </td>
                ...
                <div className="flex items-center justify-between mt-4 py-3 border-t border-gray-200">
                  <div className="flex items-center">
                    <p className="text-sm text-gray-700">
                      نمایش {((currentPage - 1) * ITEMS_PER_PAGE) + 1} تا {Math.min(currentPage * ITEMS_PER_PAGE, totalWords)} از {totalWords} لغت
                    </p>
                  </div>
                  <div className="flex items-center space-x-2 space-x-reverse">
                    <button
